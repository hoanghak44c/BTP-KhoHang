...
{
    public class frmChiTietChungTuNhapNcc2 : frmChiTiet_ChungTuNhapBase
    {
        ...
        private DataGridViewCheckBoxColumn colChkGenSerial;
        ...
        {
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(frmChiTietChungTuNhapNcc2));
            ...
            this.colChkGenSerial = new System.Windows.Forms.DataGridViewCheckBoxColumn();
            ...
            this.btnXacNhan.Text = "Sinh mã";
            this.btnXacNhan.Click += new System.EventHandler(this.btnXacNhan_Click2);
            // 
            // txtSoPhieu
            // 
            ...
            // colChkGenSerial
            // 
            this.colChkGenSerial.Frozen = true;
            this.colChkGenSerial.HeaderText = "";
            this.colChkGenSerial.Name = "colChkGenSerial";
            // 
            ...
            this.Name = "frmChiTietChungTuNhapNcc2";
        ...
        public frmChiTietChungTuNhapNcc2(frm_ListChungTuNhap frm, ChungTuXuatNhapNccInfo chungTuXuatNhapNccInfo,
        ...
        public frmChiTietChungTuNhapNcc2(int oid, string sochungtu, string ngaylap, string sopo)
            : base(oid, sochungtu, ngaylap, sopo, Declare.Prefix.PhieuNhapHangMua)
        {
            InitializeComponent();
        ...
        protected override void SaveChungTu()
        {
            ...
            if (success) AutoGenSerialMua.Instance.PrintSerial();
        ...
        private void btnXacNhan_Click2(object sender, EventArgs e)
        {
            try
            {
                var maxValue = 0;
                ...
                {
                    var item = row.DataBoundItem as ChungTuXuatNhapNccChiTietInfo;
                    ...
                    if (item != null)
                        ...
                        if (row.Cells[0].Value != null && (bool)row.Cells[0].Value)
                        {
                            ...
                            if (khoBusiness.ListChiTietHangHoa.FindAll(delegate(ChungTuNhapNccChiTietHangHoaInfo match)
                            {
                                return match.IdSanPham == item.IdSanPham;
                            }).Count == 0)
                            {
                                if (item.TrungMaVach == 1)

                                    maxValue += 1;

                                else

                                    maxValue += item.SoLuong;
                            }
                        }
                        else
                        {
                            ...
                            khoBusiness.ListChiTietHangHoa.ForEach(delegate(ChungTuNhapNccChiTietHangHoaInfo action)
                            {
                                if (action.IdSanPham == item.IdSanPham)
                                    soluong += action.SoLuong;
                            });
                            if (soluong != item.SoLuong)
                            {
                                throw new ManagedException("Bạn cần bắn đủ các mã vạch của NCC trước");
                            }
                        }
                }
                ...
                frmProgress.Instance.MaxValue = maxValue;
                ...
                frmProgress.Instance.DoWork(delegate
                {
                    try
                    {
                        foreach (DataGridViewRow row in dgvChiTiet.Rows)
                        {
                            if (row.Cells[0].Value == null || !((bool)row.Cells[0].Value)) continue;

                            var item = row.DataBoundItem as ChungTuXuatNhapNccChiTietInfo;

                            if (item == null) throw new ManagedException("Không đúng kiểu dữ liệu.");

                            AutoGenSerialMua.Instance.GenSerial(item, khoBusiness.ListChiTietHangHoa);
                        }
    ...
    public class AutoGenSerialMua : AutoGenSerial<ChungTuXuatNhapNccChiTietInfo, ChungTuNhapNccChiTietHangHoaInfo>
    {
        private static AutoGenSerialMua instance;

        protected AutoGenSerialMua() : base(Declare.MaTrungTam, Declare.IdNhanVien)
        {
            if (Declare.IdTrungTamHachToan <= 0)

                throw new ManagedException("Trung tâm hạch toán chưa được thiết lập!");

            if (Declare.IdTrungTam != Declare.IdTrungTamHachToan)

                MaTrungTam = DMTrungTamDataProvider.GetTrungTamByIdInfo(Declare.IdTrungTamHachToan).MaTrungTam;
        }

        public static AutoGenSerialMua Instance
        {
            get
            {
                if(instance == null || instance.IdNhanVien != Declare.IdNhanVien)
                    
                    instance = new AutoGenSerialMua();

                return instance;
            }
        }

        protected override ChungTuNhapNccChiTietHangHoaInfo CreateSerial(string maVach)
        {
            var serial = base.CreateSerial(maVach);

            serial.TransactionID = ChungTuChiTietInfo.TransactionID;

            return serial;
        }

        protected override bool MatchRemove(ChungTuNhapNccChiTietHangHoaInfo match)
        {
            return base.MatchRemove(match) && match.TransactionID == ChungTuChiTietInfo.TransactionID;
        }
    }
}
